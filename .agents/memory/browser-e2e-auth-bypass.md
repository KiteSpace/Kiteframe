...
8. **Routed workflow localStorage identity** — do not pre-seed project-scoped localStorage under the database project UUID before opening a routed workflow. Its initial local tab can keep a temporary project id, so create the state through the visible UI and derive the actual storage key from that write before simulating another tab's storage update.
