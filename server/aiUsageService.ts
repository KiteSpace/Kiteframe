...
const MODEL_MULTIPLIERS: Record<string, number> = {
  'gpt-4o': 100,       // 1.0x (base)
  'gpt-4o-mini': 100,  // 1.0x (same unit cost for Beta)
  'gpt-3.5-turbo': 100, // 1.0x (same unit cost for Beta)
};
...
const COST_PER_1K_TOKENS: Record<string, number> = {
  'gpt-4o': 15000,      // ~$0.015 per 1K tokens (average)
  'gpt-4o-mini': 750,   // ~$0.00075 per 1K tokens
...
export function estimateCostMicrodollars(totalTokens: number, model: string): number {
  const costPer1K = COST_PER_1K_TOKENS[model] || COST_PER_1K_TOKENS['gpt-4o'];
