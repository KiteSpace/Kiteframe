...
describe('PRD generation reliability', () => {
  ...
  it('preserves manual sections while regenerating every remaining section', async () => {
    const existing: WorkflowPRD = {
      ...
      sections: [{ id: 'overview', title: 'Overview', content: '## Overview\n\nHuman-authored overview' }],
  ...
  it('normalizes duplicate section headings across generation, refresh, and elaboration', async () => {
    const duplicateHeading = '## Failure Scenarios\n\n- Retry recoverable environment errors.';
    const client = clientFrom(async () => ({ text: duplicateHeading }));

    const generated = await generateWorkflowPRD(client, workflowModel);
    const failureSection = generated.sections.find(section => section.id === 'failure-scenarios');
    expect(failureSection?.content).toBe('- Retry recoverable environment errors.');

    const refreshed = await generateSingleSection(
      client,
      workflowModel,
      'failure-scenarios',
      generated,
    );
    expect(refreshed?.content).toBe('- Retry recoverable environment errors.');

    const elaborated = await elaborateSection(
      client,
      workflowModel,
      'failure-scenarios',
      failureSection?.content ?? '',
    );
    expect(elaborated).toBe('- Retry recoverable environment errors.');
  });
