...
export function openInReader(next: Omit<ReaderTarget, 'openedAt'>): boolean {
  if (next.docKind !== 'project-prd' && next.docKind !== 'workflow-prd') {
...
export function useIsOpenInReader(docKind: DocKind, workflowId?: string): boolean {
  const current = useReaderTarget();
  if (!current) return false;
  return current.docKind === docKind && (current.workflowId ?? undefined) === (workflowId ?? undefined);
}

/** Stable id for the document a target addresses. */
export function targetDocId(t: Pick<ReaderTarget, 'docKind' | 'workflowId'>): string {
