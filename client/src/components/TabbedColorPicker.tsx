...
export function TabbedColorPicker({
  ...
  useEffect(() => {
    ...
    if (
      firstPropertyKey &&
      !colorProperties.find((prop) => prop.key === activeTab)
    ) {
  ...
  return (
    <div
      ...
      data-testid={testIdScope ? `${testIdScope}-tabbed-picker` : undefined}
    >
      {/* Header */}
      ...
      <div className="flex flex-wrap gap-1">
        {colorProperties.map((property) => (
          <Button
            ...
            data-testid={
              testIdScope ? `${testIdScope}-tab-${property.key}` : undefined
            }
          >
            {property.label}
          </Button>
      ...
      <FigmaStyleColorPicker
        ...
        fillColor={
          activeProperty.type === "fill" ? activeProperty.value : undefined
        }
        fillOpacity={
          activeProperty.type === "fill" ? activeProperty.opacity || 100 : 100
        }
        onFillColorChange={
          activeProperty.type === "fill" ? handleColorChange : undefined
        }
        onFillOpacityChange={
          activeProperty.type === "fill"
            ? activeProperty.hasOpacity
              ? handleOpacityChange
              : () => {}
            : undefined
        }
        strokeColor={
          activeProperty.type === "stroke" ? activeProperty.value : undefined
        }
        strokeOpacity={
          activeProperty.type === "stroke" ? activeProperty.opacity || 100 : 100
        }
        onStrokeColorChange={
          activeProperty.type === "stroke" ? handleColorChange : undefined
        }
        onStrokeOpacityChange={
          activeProperty.type === "stroke"
            ? activeProperty.hasOpacity
              ? handleOpacityChange
              : () => {}
            : undefined
        ...
        testIdScope={
          testIdScope ? `${testIdScope}-${activeProperty.key}` : undefined
        }
      />
    </div>
  );
