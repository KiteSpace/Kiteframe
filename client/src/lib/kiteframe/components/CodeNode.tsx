...
const CodeNodeComponent: React.FC<CodeNodeComponentProps> = ({
  ...
  const getSnippetsForVariable = useCallback((varName: string, columns: string[]) => {
    const firstCol = columns[0] || 'column';
    const numericCol = columns.find(c => ['cost', 'price', 'amount', 'total', 'value', 'count', 'quantity'].some(n => c.toLowerCase().includes(n))) || columns[0] || 'value';
    return [
  ...
  return (
    ...
    >
            ...
            >
              ...
              {source.variableName && (
                <>
                  ...
                  <code className="px-1 py-0.5 rounded text-[10px] font-mono" style={{ backgroundColor: 'rgba(99, 102, 241, 0.25)' }}>
                    {source.variableName}
                  </code>
                </>
              )}
      ...
      {showDataReference && connectedDataSources.length > 0 && (
        <div
          className="absolute top-0 right-0 h-full w-64 bg-gray-900 border-l border-gray-700 flex flex-col overflow-hidden z-40"
